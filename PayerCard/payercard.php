...
class plgVmPaymentPayercard extends vmPSPlugin {
	...
	public function getVmPluginCreateTableSQL() {
		return $this->createTableSQL('Payment Payer Card Table');
	}
	...
	function plgVmConfirmedOrder($cart, $order) {
		if (!class_exists('payread_post_api')) {
			require_once("plugins/vmpayment/payercard/payer/payread_post_api.php");
		}
		...
		$payer->add_buyer_info($order['details']['BT']->first_name, $order['details']['BT']->last_name, $order['details']['BT']->address_1, $order['details']['BT']->address_2, $order['details']['BT']->zip, $order['details']['BT']->city, $order['details']['BT']->country, //MISSING
				$order['details']['BT']->phone_1, $order['details']['BT']->phone_2, $order['details']['BT']->phone_2, $order['details']['BT']->email);
